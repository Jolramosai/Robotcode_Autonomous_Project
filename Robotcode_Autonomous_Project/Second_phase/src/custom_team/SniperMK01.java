...
public class SniperMK01  extends TeamRobot
{
...
// The  strategy is to get as close to our target robot as possible hugging the walls and evading  the death zone
         ...
         if (state.enemy_being_attacked != null) 
			...
			if (horizontal && vertical) 
				...
				if (Math.abs(infoEnemy.position.getX()) <= Math.abs(infoEnemy.position.getY() - getY())) 
                                {
			...
			if (Math.abs(getTurnRemaining()) < 1 || Math.abs(getVelocity()) < 0.01) 
				...
				if (horizontal) 
                                {
					delta = infoEnemy.position.getX() - getX();
				 }
                                else 
                                {
					delta = infoEnemy.position.getY() - getY();
				}
				...
				// time
				if (getTime() - lastDirectionShift > 20) 
                                { 
	...
	//currently finds the weakest enemy
      ...
      ///////////////                                  ///////////////         
        ...
        public void goTo (int x, int y)  //vai calcular e direcionar o tanque reorrendo a fun�oes trigonometricas
        {
    	double a;
